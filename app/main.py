            ...
            try:
                ...
                if response.status_code == 200:
                    if data["status"] == "OK":
                        ...
                        return {
                            "error": f"Geocoding API response status: {data['status']}"
                        }
                else:
                    return {
    ...
    try:
        ...
        if mode == "address":
            ...
            for call in payload["calls"]:
                result = await get_statsector(address=call[0])
            ...
            results = []
            for call in payload["calls"]:
                result = await get_statsector(lat=call[0], lon=call[1])
